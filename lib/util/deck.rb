...
class Deck
	...
	def create_deckfile(filename)
		#card_type,card_name,quantity,price
		@log.debug "create_deckfile(" + filename + ") start."
		previous_card_type = ""
		File.open(filename, "w:sjis") do |file|
			file.print @deckname.to_s + " " + @price_of_all.to_s + "\n"
		
			@cards.each do |card|
			
				#set separator
				if previous_card_type == "land" and card.card_type == "creature"
					file.print @quantity_of_lands.to_s + " Lands " + @price_of_lands.to_s + "\n"
				elsif previous_card_type == "creature" and card.card_type == "spell"
					file.print @quantity_of_creatures.to_s + " Creatures " + 	@price_of_creatures.to_s + "\n"
				elsif previous_card_type == "spell" and card.card_type == "sideboardCards"
					file.print @quantity_of_spells.to_s + " Spells " + @price_of_spells.to_s + "\n"
					file.print @quantity_of_mainboard_cards.to_s + " MainboardCards " + @price_of_mainboard_cards.to_s + "\n"
				elsif previous_card_type == "sideboardCards" and card.card_type == "spell"
					file.print @quantity_of_sideboard_cards.to_s + " SideboardCards " + @price_of_sideboard_cards.to_s + "\n"
				end

				#write decks
				file.print card.card_type + "," + card.name + "," + card.quantity + "," + card.price.to_s + "," + card.store_url + "," + card.price.date.to_s + "\n"
				
				previous_card_type = card.card_type
			end	
			file.print @quantity_of_sideboard_cards.to_s + " SideboardCards\n"
			file.print @path.to_s
		end
	end
	...
	def read_deckfile(filename)
		...
		File.open(filename, "r:sjis").each do |line|
			...
			#if card_type.include?("MainboardCards") then puts "space" end
			#puts card_type
			#puts date
		end
	end
